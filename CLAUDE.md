...
- `.env.example` (in `apps/server`) documents required vars: `JWT_SECRET`, `EMQX_SYSTEM_PASSWORD`, `PLAYER_ID_SALT`, `ADMIN_SECRET`, `DATABASE_URL`, Steam/Discord OAuth vars, `CHAT_ENABLED`, `TESTING_MODE`, `BET_MOD_INDEX_URL`, `LAUNCHER_INTEGRITY_SECRET`.
