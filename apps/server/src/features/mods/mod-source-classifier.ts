...
const RELEASE_ASSET_LATEST =
	/^https:\/\/github\.com\/([^/]+)\/([^/]+)\/releases\/latest\/download\/(.+)$/
const RELEASE_ASSET_TAGGED =
	/^https:\/\/github\.com\/([^/]+)\/([^/]+)\/releases\/download\/([^/]+)\/(.+)$/
const TAG_ARCHIVE =
	/^https:\/\/github\.com\/([^/]+)\/([^/]+)\/archive\/refs\/tags\/(.+)\.zip$/
// Legacy shape (no "refs/" prefix) -- still present on some older mods.
const TAG_ARCHIVE_LEGACY =
	/^https:\/\/github\.com\/([^/]+)\/([^/]+)\/archive\/(.+)\.zip$/
const TAG_ARCHIVE_CODELOAD =
...
// True for a URL whose bytes change as the source moves on: a branch archive
// (tracks the branch head) or a "latest release" asset pointer. A ranked pin
// hashes what the URL served at pin time, so once such a mod's version moves
// on, an older version row's stored URL no longer serves the bytes that
// were hashed -- the pin must be cleared, not kept.
export function isMovingDownloadUrl(url: string): boolean {
	return BRANCH_ARCHIVE.test(url) || RELEASE_ASSET_LATEST.test(url)
}
...
export function resolveReliableDownloadUrl(url: string): string {
	...
	const tagMatch =
		url.match(TAG_ARCHIVE) ??
		url.match(TAG_ARCHIVE_LEGACY) ??
		url.match(TAG_ARCHIVE_CODELOAD)
	if (tagMatch) {
