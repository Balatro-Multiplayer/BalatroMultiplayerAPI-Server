...
if (bearerTokens.length === 0 && process.env.NODE_ENV === 'production') {
	console.error(
		'[moderation] MODERATION_BEARER_TOKEN produced zero usable tokens in production — refusing to boot unauthenticated',
	)
	process.exit(1)
}
if (!GUARD_MODEL) {
...
// Human-edited game-vocabulary notes (GUARD_CONTEXT_PATH): plain text, `#`
// comments stripped, injected into every guard prompt. Editing the file +
// restart is the cheapest way to fix a false-positive PATTERN ("white stake"
// read as racial) — no retrain, no rebuild, generalizes to any phrasing.
let domainContext: string | undefined
let guardContextStatus: 'set' | 'unset' | 'error' = 'unset'
if (process.env.GUARD_CONTEXT_PATH) {
	const { readFileSync } = await import('node:fs')
	const { parseDomainContext } = await import('./guard/prompt.js')
	try {
		domainContext = parseDomainContext(
			readFileSync(process.env.GUARD_CONTEXT_PATH, 'utf8'),
		)
		guardContextStatus = 'set'
		console.error(
			`[moderation] guard domain context loaded: ${domainContext.length} chars`,
		)
		// The tuned-v2 model shipped in docker-compose.prod.yml was trained BARE
		// (game vocabulary already baked into its weights) — injecting domain
		// context on top would shift it off that training distribution. This
		// path is only safe for a stock/untuned model; confirm before enabling
		// it against tuned-v2.
		console.error(
			'[moderation] WARNING: GUARD_CONTEXT_PATH is set — confirm the loaded GUARD_MODEL was NOT trained with this vocabulary baked in (tuned-v2 was), or this degrades its verdicts.',
		)
	} catch (err) {
		guardContextStatus = 'error'
		console.error(
			`[moderation] failed to read GUARD_CONTEXT_PATH (${String(err)}) — continuing without domain context`,
		)
		failIfListsRequired('GUARD_CONTEXT_PATH', err)
	}
}
...
const server = createModerationServer({
	...
	lists: {
		...
		guardContext: guardContextStatus,
	},
})
