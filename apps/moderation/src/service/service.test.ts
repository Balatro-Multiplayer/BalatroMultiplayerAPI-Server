...
describe('createModerationService', () => {
	...
	it('a deadline-slow guard fails CLOSED: rejects with guard_unavailable + retry hint', async () => {
		...
		expect(r.retryAfterMs).toBe(DEFAULT_POLICY.guardRetryAfterMs)
	})

	it('rate limits a burst from one player, isolated per player', async () => {
		const clock = () => 1000 // frozen: no refill between calls
		const { guard, judged } = recordingGuard()
		...
		const sixth = await s.moderate(req('msg 6'))
		expect(sixth.band).toBe('rate_limited')
		expect(sixth.retryAfterMs).toBeGreaterThan(0)
		expect(judged).toHaveLength(5) // exactly one judgement per allowed message
		const other = await s.moderate(req('hello', 'p2'))
		expect(other.band).toBe('clean')
	})

	it('does not burn a guard judgement on a rate-limited message (regression)', async () => {
		const clock = () => 1000 // frozen: no refill between calls
		const { guard, judged } = recordingGuard()
		const s = createModerationService({ guard, clock })
		for (let i = 0; i < 5; i++) {
		...
		expect(sixth.band).toBe('rate_limited')
		expect(sixth.retryAfterMs).toBeGreaterThan(0)
		// The 6th message was already decided by rate limit — the guard must
		// never have been asked to judge it.
		expect(judged).toHaveLength(5)
	})

	it('a rate-limited rejection spends no token and is not double-consumed', async () => {
		let t = 1000
		const clock = () => t
		...
		// One refill interval later (0.5 tok/s -> +1 token): exactly ONE further
		// message is allowed. If the peek had double-consumed, zero would be
		// allowed here; if the peek had consumed instead of the core, two would.
		t += 2000
		const seventh = await s.moderate(req('msg 7'))
		expect(seventh.band).toBe('clean')
		const eighth = await s.moderate(req('msg 8'))
		expect(eighth.band).toBe('rate_limited')
	})
