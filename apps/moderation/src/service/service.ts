...
import { canConsume, newBucket } from '../pipeline/rate-limit.js'
...
export function createModerationService(
	...
	// The ONLY per-player state, and it is EPHEMERAL BY DESIGN: in-process token
	// buckets that protect the single guard lane from one spammer. Losing them on
	// restart is correct — a restart hands everyone a fresh burst, which is
	// strictly safer than the alternative of persisting them. Nothing else about a
	// player is remembered; two instances judge the same message identically.
	...
	return {
		...
		async moderate(req: ModerateRequest): Promise<ModerateResponse> {
			...
			// PEEK before judging. `canConsume` returns a boolean and no bucket, so
			// it cannot spend a token: decideModeration below stays the ONE consume
			// site. Without this, a spammer burns a ~300ms guard slot per message on
			// a verdict that was already predetermined (rate limit precedes every
			// content tier), starving the single lane for everyone else.
			const rateLimited = !canConsume(
				buckets.get(req.playerId) ?? newBucket(policy.rateLimit, started),
				policy.rateLimit,
				started,
			)

			// An explicit skip marker, never a bare null: decideModeration's
			// fail-closed branch keys off `{skipped}`, so a peek/consume mismatch
			// (e.g. a rewound bucket) still rejects instead of silently allowing.
			const guard: GuardInput = rateLimited
				? { skipped: 'rate_limited' }
				: deterministicOutcome
					? { skipped: 'deterministic' }
					: await lane.judge(guardText(text), policy.guardDeadlineMs)

			// Re-read AFTER the await, deliberately: a concurrent message from the
			// same player may have spent a token while we judged. THIS bucket is the
			// one the core consumes, so concurrency behaves exactly as it did before
			// the short-circuit existed. The peek above can never be a false positive:
			// a concurrent writer can only remove tokens and only move lastRefillMs
			// forward, and refill() adds nothing when nowMs <= lastRefillMs.
			const bucket =
				buckets.get(req.playerId) ?? newBucket(policy.rateLimit, started)
			...
			buckets.set(req.playerId, decision.newBucket)
