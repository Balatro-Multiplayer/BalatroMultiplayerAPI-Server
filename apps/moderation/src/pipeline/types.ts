...
export type ModerationPolicy = {
	 ...
	 * rejected with band `guard_unavailable` + retryAfterMs (owner's rule:
	 * nothing unjudged ever appears in chat). 0/negative = wait indefinitely.
	 */
	guardDeadlineMs: number
	/**
	 * retryAfterMs returned with a `guard_unavailable` rejection — the client's
	 * hint for when the judgement lane is likely to have drained enough to try
	 * again.
	 */
	guardRetryAfterMs: number
	/**
...
export type Decision = {
	...
	/** Present when band is `rate_limited`. */
	retryAfterMs?: number
}
