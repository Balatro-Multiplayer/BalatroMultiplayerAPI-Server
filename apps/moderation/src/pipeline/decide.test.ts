...
const FULL_BUCKET: TokenBucket = { tokens: 5, lastRefillMs: 0 }
...
describe('decideModeration — precedence', () => {
	...
	it('rate limit beats content checks', () => {
		const d = decideModeration(
			input({
				bucket: { tokens: 0, lastRefillMs: 1000 },
				isAllowlisted: true,
			}),
		)
		expect(d.band).toBe('rate_limited')
		expect(d.retryAfterMs).toBeGreaterThan(0)
	})

	it('rate limit is the FIRST gate in the /moderate shape', () => {
		const d = decideModeration(
			input({
				bucket: { tokens: 0, lastRefillMs: 1000 },
				isAllowlisted: true,
		...
		expect(d.decision).toBe('reject')
		expect(d.retryAfterMs).toBeGreaterThan(0)
		// refilled-but-unspent: an empty bucket at the same instant refills to 0
		// (no elapsed time) and stays 0 — no token was taken from a rejected message.
		expect(d.newBucket.tokens).toBe(0)
	})
...
describe('decideModeration — guard absent / skipped (fail-closed)', () => {
	...
	it('rejects with guard_unavailable + retry hint when the judgement hit the deadline', () => {
		const g: GuardInput = { skipped: 'deadline' }
		const d = decideModeration(input({ guard: g }))
		expect(d.decision).toBe('reject')
		expect(d.band).toBe('guard_unavailable')
		expect(d.verdict.guardSkipped).toBe('deadline')
		expect(d.retryAfterMs).toBe(ENFORCE.guardRetryAfterMs)
	})
	...
	it('keeps the rate-limit token spent on guard_unavailable so retries stay capped during an outage', () => {
		const bucket: TokenBucket = { tokens: 3, lastRefillMs: 1000 }
		const d = decideModeration(
			input({ bucket, nowMs: 1000, guard: { skipped: 'deadline' } }),
		)
		expect(d.band).toBe('guard_unavailable')
		expect(d.newBucket.tokens).toBe(2)
	})

	it('rate-limits a player retrying through a full guard outage', () => {
		// Refunding here would make the bucket immortal: every retry re-enters
		// the lane and deepens the backlog that caused the outage.
		let bucket: TokenBucket = {
			tokens: ENFORCE.rateLimit.burst,
			lastRefillMs: 0,
		}
		const bands: Band[] = []
		for (let i = 0; i < ENFORCE.rateLimit.burst + 3; i++) {
			const d = decideModeration(
				input({ bucket, nowMs: 0, guard: { skipped: 'backlog' } }),
			)
			bands.push(d.band)
			bucket = d.newBucket
		}
		expect(bands).toContain('rate_limited')
	})
			...
			fc.property(
				...
				(message, nowMs, g, isAllowlisted, shadowMode, safetySignals) => {
					const d = decideModeration({
						message,
						nowMs,
						bucket: { tokens: 5, lastRefillMs: 0 },
						isAllowlisted,
						threatMatches: [],
						obscenityMatches: [],
						safetySignals,
					...
					expect(d.newBucket.tokens).toBeGreaterThanOrEqual(0)
